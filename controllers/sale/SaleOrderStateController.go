...
func (ctl *SaleOrderStateController) SaleOrderStateList(query map[string]string, fields []string, sortby []string, order []string, offset int64, limit int64) (map[string]interface{}, error) {

	var arrs []md.SaleOrderState
	paginator, arrs, err := md.GetAllSaleOrderState(query, fields, sortby, order, offset, limit)
...
func (ctl *SaleOrderStateController) PostList() {
	...
	if result, err := ctl.SaleOrderStateList(query, fields, sortby, order, offset, limit); err == nil {
		ctl.Data["json"] = result
	}
	ctl.ServeJSON()
