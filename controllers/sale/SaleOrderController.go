...
func (ctl *SaleOrderController) SaleOrderList(query map[string]string, fields []string, sortby []string, order []string, offset int64, limit int64) (map[string]interface{}, error) {

	var arrs []md.SaleOrder
	paginator, arrs, err := md.GetAllSaleOrder(query, fields, sortby, order, offset, limit)
...
func (ctl *SaleOrderController) PostList() {
	...
	if result, err := ctl.SaleOrderList(query, fields, sortby, order, offset, limit); err == nil {
		ctl.Data["json"] = result
	}
	ctl.ServeJSON()
