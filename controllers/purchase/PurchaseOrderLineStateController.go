...
func (ctl *PurchaseOrderLineStateController) PurchaseOrderLineStateList(query map[string]string, fields []string, sortby []string, order []string, offset int64, limit int64) (map[string]interface{}, error) {

	var arrs []md.PurchaseOrderLineState
	paginator, arrs, err := md.GetAllPurchaseOrderLineState(query, fields, sortby, order, offset, limit)
...
func (ctl *PurchaseOrderLineStateController) PostList() {
	...
	if result, err := ctl.PurchaseOrderLineStateList(query, fields, sortby, order, offset, limit); err == nil {
		ctl.Data["json"] = result
	}
	ctl.ServeJSON()
