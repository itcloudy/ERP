...
func (ctl *PurchaseOrderStateController) PurchaseOrderStateList(query map[string]string, fields []string, sortby []string, order []string, offset int64, limit int64) (map[string]interface{}, error) {

	var arrs []md.PurchaseOrderState
	paginator, arrs, err := md.GetAllPurchaseOrderState(query, fields, sortby, order, offset, limit)
...
func (ctl *PurchaseOrderStateController) PostList() {
	...
	if result, err := ctl.PurchaseOrderStateList(query, fields, sortby, order, offset, limit); err == nil {
		ctl.Data["json"] = result
	}
	ctl.ServeJSON()
