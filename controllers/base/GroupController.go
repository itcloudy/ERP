...
// 获得符合要求的城市数据
func (ctl *GroupController) groupList(query map[string]interface{}, exclude map[string]interface{}, condMap map[string]map[string]interface{}, fields []string, sortby []string, order []string, offset int64, limit int64) (map[string]interface{}, error) {

	var groups []md.Group
	paginator, groups, err := md.GetAllGroup(query, exclude, condMap, fields, sortby, order, offset, limit)
...
func (ctl *GroupController) PostList() {
	...
	if result, err := ctl.groupList(query, exclude, cond, fields, sortby, order, offset, limit); err == nil {
		ctl.Data["json"] = result
	}
	ctl.ServeJSON()
